...
module.exports.handler = async (event, context, callback) => {
  ...
  try{
    ...
    const promises = resultList.map(async (result, index)=>{
      const {decoded, abi, log, contractAddress} = result;
      //console.log(result);
...
async function printEventLog(contractWapper, resultList){
  //console.log("printEventLog", resultList);
  const results = resultList.map(async (result, index)=>{
    const {decoded, abi, log, contractAddress} = result;
    ...
    console.log("\r\n new item", index, JSON.stringify(item));
